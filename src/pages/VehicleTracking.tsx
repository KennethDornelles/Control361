            ...
            <main className="container mx-auto px-8 pb-6">
                <div className="mb-6 border-b border-[#29292E]">
                    <div className="flex space-x-8">
                        <button
                            className={`px-1 py-4 font-medium text-base ${activeTab === 'mapa'
                        ...
                        <button
                            className={`px-1 py-4 font-medium text-base ${activeTab === 'lista'
                                ? 'text-[#8257E5] border-b-2 border-[#8257E5] -mb-px'
                                : 'text-[#7C7C8A] hover:text-white'
                                }`}
                ...
                {error && (
                    <div className="bg-[#F75A68] bg-opacity-10 border border-[#F75A68] text-white px-4 py-3 rounded mb-6" role="alert">
                        <p>{error}</p>
                    </div>
                )}
                    ...
                    <div className="bg-[#1A1A1D] rounded-md overflow-hidden border border-[#29292E]">
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-[#29292E]">
                                <thead>
                                    <tr>
                                        <th className="px-6 py-4 text-left text-xs font-medium text-[#C4C4CC] uppercase tracking-wider">Placa</th>
                                        <th className="px-6 py-4 text-left text-xs font-medium text-[#C4C4CC] uppercase tracking-wider">Frota</th>
                                        <th className="px-6 py-4 text-left text-xs font-medium text-[#C4C4CC] uppercase tracking-wider">Tipo</th>
                                        <th className="px-6 py-4 text-left text-xs font-medium text-[#C4C4CC] uppercase tracking-wider">Modelo</th>
                                        <th className="px-6 py-4 text-left text-xs font-medium text-[#C4C4CC] uppercase tracking-wider">Status</th>
                                    </tr>
                                </thead>
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                                                    ${vehicle.status === 'EM MOVIMENTO' ? 'bg-[#00875F] text-white' :
                                                        vehicle.status === 'MANUTENÇÃO' ? 'bg-[#FBA94C] text-[#121214]' :
                                                            vehicle.status === 'INATIVO' ? 'bg-[#F75A68] text-white' :
                                                                vehicle.status === 'PARADO' ? 'bg-[#81D8F7] text-[#121214]' :
                                                                    vehicle.status === 'ATIVO' ? 'bg-[#00875F] text-white' :
